...
export default function CoverImageModal({
  ...
  const handlePieceImageClick = async (imageUrl: string) => {
    if (isGif(imageUrl)) {
      onSelect(imageUrl);
      return;
    }
    setCropSrc(imageUrl);
    setTab("upload"); // switch to upload tab to show cropper
  };
    ...
    >
      ...
      >
        {(["upload", "piece"] as Tab[]).map((t) => (
          ...
          >
            {t === "upload" ? "Upload" : "From your piece"}
          </button>
        ))}
      </div>
      ...
      {cropSrc ? (
        ...
        <>
          {pieceImages.length === 0 ? (
            ...
            >
              {pieceImages.map((url, i) => (
                <button
                  key={i}
                  onClick={() => handlePieceImageClick(url)}
                  style={{
                    aspectRatio: "1",
                    border:
