...
- [x] Club Cadence: creation flow picks a cadence and seeds a real first riff — shipped. Follow-up (separate, schema-gated): persisting `Cadence` on Club (schema proposal handed to Kyle) and wiring up the disabled "Riff cadence" settings item once it lands
